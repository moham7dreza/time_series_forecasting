...
class Multivariant_Multiple_Input_CNN:
    ...
    def simple_LSTM_model(self):
        ...
        model.add(
            LSTM(
                128, return_sequences=True, input_shape=(self.n_steps, self.n_features)
            )
        )
    ...
    def simple_GRU_model(self):
        model = Sequential()
        model.add(
            GRU(128, return_sequences=True, input_shape=(self.n_steps, self.n_features))
        )
        model.add(Dropout(0.2))
    ...
    def multi_headed_cnn_model(self):
        ...
        cnn2 = Flatten()(cnn2)
        # third input model
        visible3 = Input(shape=(self.n_steps, self.n_features))
    ...
    def prepare_dataset(self, *input_sequences):
        # Ensure at least one input sequence is provided
        if not input_sequences:
            raise ValueError("At least one input sequence is required")

        # Convert each input sequence to a column vector
        input_arrays = [array(seq).reshape((len(seq), 1)) for seq in input_sequences]

        # Horizontally stack columns
        dataset = hstack(tuple(input_arrays))
    ...
    def download_data(self):
        ...
        combined_seq = hstack([seq.reshape(len(seq), 1) for seq in sequences])
        # input_arrays = [array(seq).reshape((len(seq), 1)) for seq in sequences]

        # combined_seq = hstack(tuple(input_arrays))

        return combined_seq

    def download_next_timestep(self):
        sequences = []
        for ticker in self.tickers:
            # Download data from Yahoo Finance
            data = yf.download(ticker, start="2023-10-23", end="2023-11-24")
    ...
    def run_multi_headed_cnn_model(self):
        ...
        X3 = self.X[:, :, 2].reshape(self.X.shape[0], self.X.shape[1], self.n_features)

        # Define and compile the model
        self.model = self.multi_headed_cnn_model()

        # Fit the model
        self.train_model(X=[X1, X2, X3])

        # self.input = array([[80, 85], [90, 95], [100, 105]])
        # self.input = array([[80, 85, 95], [90, 95, 105], [100, 105, 115]])

        x1 = self.input[:, 0].reshape((1, self.n_steps, self.n_features))
        x2 = self.input[:, 1].reshape((1, self.n_steps, self.n_features))
        x3 = self.input[:, 2].reshape((1, self.n_steps, self.n_features))

        yhat = self.model.predict([x1, x2, x3], verbose=0)
    ...
    def run_simple_rnn_model(self):
        # the dataset knows the number of features, e.g. 2
        self.n_features = self.X.shape[2]

        # Define and compile the model
        self.model = self.simple_RNN_model()
    ...
    def run_simple_gru_model(self):
        # the dataset knows the number of features, e.g. 2
        self.n_features = self.X.shape[2]

        # Define and compile the model
        self.model = self.simple_GRU_model()
    ...
    def run_app(self):
        keys = ["CNN", "MH CNN", "Stacked LTSM", "BiLSTM", "LSTM", "GRU", "RNN"]
        ...
        results = [
            ...
            self.run_simple_gru_model(),
            self.run_simple_rnn_model(),
        ]
        flatted_results = self.flatten_arr(results)
        print(flatted_results)
        results = [model for model in flatted_results]
...
x = [model.download_next_timestep()[-4:-1]]
print(x)
model.input = array(x)
# Run the models
model.run_app()
